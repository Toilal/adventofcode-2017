...
"""
regex = re.compile('^(?P<register>.+?) (?P<operator>.+?) (?P<operand>.+?) if (?P<c_register>.+?) (?P<c_operator>.+?) (?P<c_operand>.+?)$')
...
def parse(line):
    m = regex.match(line)
    parsed = m.groupdict()
    if parsed['register'] not in registers:
        registers[parsed['register']] = 0
    if parsed['c_register'] not in registers:
        registers[parsed['c_register']] = 0
    parsed['operand'] = int(parsed['operand'])
    parsed['c_operand'] = int(parsed['c_operand'])
    return parsed

def should_execute(parsed):

    eval_locals = {
        'l': registers[parsed['c_register']],
        'r': parsed['c_operand']
    }


    return eval('l ' + parsed['c_operator'] + ' r', {}, eval_locals)

def execute(parsed):
    if parsed['operator'] == 'inc':
        registers[parsed['register']] += parsed['operand']
    elif parsed['operator'] == 'dec':
        registers[parsed['register']] -= parsed['operand']
